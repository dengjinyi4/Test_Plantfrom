...
def getorderreson(mydit):
    ...
    orderreson={"A":"同一用户cookie24小时内重复展现","B":"订单无效或当前时间不可投放","C":"今日没有预算","D":"预算已不足","E":"投放状态错误（冻结或暂停或结束）","F":"出价过低","G":"订单对应广告主无效或状态错误","H":"订单对应广告计划无效或错误",
                ...
                "b":"广告订单媒体广告位定向不符","c":"媒体不可以投放百度广告,活动不支持百度广告","d":"广告位高级屏蔽","e":"小程序广告在非小程序广告位","f":"非小程序广告在小程序广告位","g":"出价过低","h":"活动定制过滤","j":"订单没有坑位尺寸的创意",
                "k":"托底广告主过滤","l":"商品不存在或不支持cps","m":"商品不在推广期","n":"商品CPS账户保证金账户余额不足","o":"不是微信游戏配置订单",
                "p":"不是指定的广告订单","q":"ocpa出价过低","r":"ocpa暂停","s":"媒体不支持ocpa","t":"被订单的地域限制(后台)过滤","u":"不在广告位出价范围内","v":"媒体不支持A3A5广告主通投订单","w":"未找到对应cvr","x":"旧A3A5定向订单"}
    # 分组未出订单原因 字典
...
def orderbylog(zclk):
    ...
    es=Elasticsearch(["221.122.127.41:9200","221.122.127.211:9200","221.122.127.203:9200","221.122.127.126:9200","221.122.127.127:9200"]);
...
if __name__ == '__main__':
    ...
    tmpdit=orderbylognew('B3J3JDC01IOPLHE3C4')
    print tmpdit
    # print len(tmpdit)
    # for i in tmpdit:
    #     print i['tad']
