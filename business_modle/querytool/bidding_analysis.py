...
def getorderreson(mydit):
    ...
    orderreson={"A":"同一用户cookie24小时内重复展现","B":"订单无效或当前时间不可投放","C":"今日没有预算","D":"预算已不足","E":"投放状态错误（冻结或暂停或结束）","F":"出价过低","G":"订单对应广告主无效或状态错误","H":"订单对应广告计划无效或错误",
                ...
                "v":"媒体不支持A3A5广告主通投订单","w":"未找到对应cvr","x":"旧A3A5定向订单","y":"此广告位在该ocpa订单投放范围之外"}
    # 分组未出订单原因 字典
...
def orderbylognew(zclk,env):
    # es=Elasticsearch([{"host":"221.122.127.41"}],port=9200);
    if env=='dev':
        # es=Elasticsearch(["221.122.127.41:9200","221.122.127.211:9200","221.122.127.203:9200","221.122.127.127:9200",]);
...
if __name__ == '__main__':
    ...
    tmpdit=orderbylognew('B0H2UDC11ISOOLEA8X','dev')
    print tmpdit
    # print len(tmpdit)
    # for i in tmpdit:
    #     print i['tad']
