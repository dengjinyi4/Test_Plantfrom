...
def order_reson():
    tmp={"A":"同一用户cookie24小时内重复展现",
         ...
         "w":"未找到对应cvr",
         "x":"旧A3A5定向订单",
         "y":"此广告位在该ocpa订单投放范围之外",
         "z":"强推订单不在非强推坑位出现",
         "A1":"ocpa未找到对应首效果类型",
         "A2":"不是纸巾宝指定广告类型",
         "A3":"微信流量过滤支付宝广告",
         "A4":"ocpa教育客户过滤非综合支付媒体",
         "A5":"北上广深杭过滤棋牌",
         "A6":"加粉订单在新媒体上消耗超过10%",
         "TT1":"媒体id=2759上的活动id=710，用户点击未成年人签，抽奖时，过滤掉加粉类型的广告",
         "A7": "广告位不接受低于1毛的定向订单",
         "A8":"广告主A1出价大于等于0.2元",
         "A9":"广告主A1在IOS出价大于等于0.3元",
         "A10":"广告主A3出价大于等于0.5元",
         "A11":"广告主A3在IOS出价大于等于0.6元",
         "A12":"广告主A5出价大于等于0.6元",
         "B1":"内部推广坑位定向,5,6,7坑位只展示指定订单",
         "B2":"内部推广坑位定向,订单只能在5,6,,7,8坑位展示",
         "B3":"该坑位不支持展示广点通广告",
         "B4":"直跳活动不出现广点通广告",
         "B5":"活动模版不支持展示广点通广告",
         "B6":"域名配置信息不支持广点通广告",
         "B7":"订单实时消耗大于预期消耗，不满足OCPA扩量条件",
         "B8":"OCPA扩量订单,效果数为0时，只能在指定广告位出现:",
         "B9": "OCPA扩量订单,排除按 cps结算广告位,实时消耗已达标,（实时消耗—预期消耗）/预期消耗=",
         "B10": "非智能增量的ocpa通投订单，不可投放",
         "B11": "广告订单不存在",
         "B12": "被坑位13‘积分墙跳转’过滤",
         "B13":"效果个数已达订单上限，智能增量订单过滤初始广告位",
         "C1": "仅允许内部客户投放cpm",
         "C2": "cpm定向且支持CPM低价投放广告主订单出价过低",
         "C3": "cpm定向托底广告主订单出价过低",
         "C4": "cpm定向强推订单出价过低",
         "C5": "cpm定向订单默认出价过低",
         "C6": "cpm通投且支持CPM低价投放广告主订单出价过低",
         "C7": "cpm通投托底广告主订单出价过低",
         "C8": "cpm通投强推订单出价过低",
         "C9": "cpm通投订单默认出价过低",
         "C10":"ocpa出价过高",
         "C11": "该位置不支持展示激励视频广告",
         "C12": "种水果新人礼包坑位不支持激励视频广告",
         "C13": "种水果离线奖励坑位不支持激励视频广告",
         "C14": "不符合OCPA订单位次要求",
         "C15": "京东订单必须是审核通过的订单",
         "C16": "非京东送审订单",
         "C17": "京东订单不属于当前京东广告位",
         "C18": "勾选了付费坑位的广告订单只能出现在付费坑位上",
         "C19": "付费坑位上只能出勾选该坑位的广告订单",
         #20200430 SDK需求，变更为F类型的过滤原因
         # "C20": "广告位不支持任何广告类型",
         # "C21": "广告位屏蔽图片广告类型",
         # "C22": "广告位屏蔽视频广告类型",
         # "C23":"不支持sdk广点通广告",
         ...
         "D24":"APPSDK第三方广告主无广告",
         "D34":"易积分_积分兑换_免费兑换坑位指定广告",
         "D35":"易积分_任务墙坑位指定广告",
         "D36":"易积分_任务墙没有效果类型",
         "D37":"易积分_任务墙level不匹配",
         "D101":"非直跳100活动不展示6463淘宝订单",
         "E1":"手机价位与订单不符",
         "E2":"小草莓广告位7092直跳活动写死6361广告主",
         "F1": "SDK媒体的广点通应用id不存在",
         "F2": "SDK广告位的广点通应用id不存在",
         "F3": "SDK媒体的穿山甲应用id不存在",
         "F4": "SDK广告位的穿山甲应用id不存在",
         "F5": "SDK第三方广告主只能展示在app_sdk的广告位",
         "F6": "SDK第三方视频不支持此活动模板",
         "F7": "广告位屏蔽图片广告类型",
         "F8": "广告位屏蔽视频广告类型",
         "F9": "广告位不支持任何广告类型",
...
def orderbylognew(zclk,env):
	...
	"query": {
        ...
        # body['query']["bool"]['must'][0]['query_string']['query']="path:bidding AND message:\"{0}\"".format(zclk)
        body['query']["bool"]['must'][0]['query_string']['query']="message:\"{0}\"".format(zclk)
    if env=='test':
    ...
    # 测试用
    # res = es.search(index="filebeat-*", body=body)
    # print res
    tmplist=[]
    for hit in res["hits"]["hits"]:
...
def allorderdit(begindate='',enddate='',adzone_id='',ad_order_id='',myenv='',adzone_click_id='',pos='',region='',iszhitiao='0'):
    # 筛选的所有广告位点击id
    if len(adzone_click_id)>0:
        ...
        adzone_click_ids=alladzonetup(begindate,enddate,adzone_id,ad_order_id,myenv,region,iszhitiao)
    myfad={}
    tadcount=0
    wadcount=0
    mydit={}
    ...
    for i in adzone_click_ids:
        ...
        # 增加坑位等筛选条件，在tmpdit中筛选
        if len(pos)>=1:
            tmpdit=gettmpditbybid(tmpdit,'pos',pos)
        # key为订单过滤原因 value为数量
        for dit in tmpdit:
            # print type(dit)
            # print dit['tmpdit']
            ...
            # 判断订单是否于在选中列表中，如果有就+1 如果没有还需要判断下个数是否大0 如果大于0则不动，否则置为0
    ...
    return mydit
def alladzonetup(begindate,enddate,adzone_id,ad_orderid,myenv,region,iszhitiao='0'):
    tmptable=begindate[0:10].replace('-','')
    # print tmptable
    if iszhitiao=='1':
        # 适用场景：直跳没有广告，查找谢谢参与的广告位点击id，查看订单没有被选中的原因。
        tablenname='lottery_click_log'
        sql='''SELECT adzone_click_id FROM voyagerlog.{0}{1} where create_time>'{2}'
         and create_time<'{3}' and adzone_id={4} and act_award_type=6  and region like '%{5}%' limit 10'''.format(tablenname,tmptable,begindate,enddate,adzone_id,region)
    elif iszhitiao=='0':
        tablenname='ad_show_log'
        sql='''SELECT adzone_click_id FROM voyagerlog.{0}{1} where create_time>'{2}'
         and create_time<'{3}' and adzone_id={4} limit 10'''.format(tablenname,tmptable,begindate,enddate,adzone_id)
        print sql
    if myenv=='dev':
...
def getadvertiser_deduction(day,orderid):
    ...
    return result
# 根据bid条件筛选
def gettmpditbybid(tmpdit,type,para):
    mydit=[]
    if type=='pos':
        for tmp in tmpdit:
            if str(tmp['bid']['pos'])  in para:
                mydit.append(tmp)
    return mydit
if __name__ == '__main__':
    ...
    # print tmpdit
    result=alladzonetup('2020-03-25 11:25:03','2020-03-25 11:25:10',6415,1,'test','北京')
    # result=allorderdit('2019-03-15 11:25:03','2019-03-15 11:25:10','3397','30473','dev',['B3J3JDC11K5H8LC0TE'],[1,2])
    print result
    # print '1111111111111'
