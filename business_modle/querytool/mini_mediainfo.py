...
#查询授权用户数
    ...
    def authorize_user(self):
        ...
        sql=''' SELECT count(1) FROM voyager.wx_user_info  WHERE  source_id = '{}' AND  create_time>'{} 00:00:00' AND  create_time<'{} 23:59:59'   AND nick_name IS NOT NULL  AND open_id IN(SELECT DISTINCT open_id FROM voyager.`wx_user_authorize_log` WHERE TYPE =1 AND operate=1 ) ORDER BY id DESC '''.format(self.source_id,self.begin_time,self.begin_time)
...
###查询领取微信步数的用户数
    ...
    def wxstep_user(self):
        ...
        sql=''' SELECT count(DISTINCT(open_id)) FROM voyager.`wx_steplogs` WHERE open_id IN (SELECT open_id FROM wx_user_info  WHERE   source_id = '{}' AND  create_time>'{} 00:00:00' AND   create_time<'{} 23:59:59'   AND nick_name IS NOT NULL  AND open_id IN(SELECT DISTINCT open_id FROM voyager.`wx_user_authorize_log` WHERE TYPE =1 AND operate=1 ))
...
#查询多少授权用户邀请到了好友
    def invite_user(self):
        ...
        sql=''' SELECT count(DISTINCT inviter_openid) FROM `wx_invite_relation` WHERE inviter_openid IN (SELECT open_id FROM wx_user_info  WHERE   source_id = '{}' AND  create_time>'{} 00:00:00' AND   create_time<'{} 23:59:59' AND nick_name IS NOT NULL  AND open_id IN(SELECT DISTINCT open_id FROM `wx_user_authorize_log` WHERE TYPE =1 AND operate=1 ))
...
###查询授权用户邀请到了多少好友
    def invited_user(self):
          ...
          sql='''SELECT count(DISTINCT open_id) FROM `wx_invite_relation` WHERE inviter_openid IN (SELECT open_id FROM wx_user_info  WHERE   source_id = '{}'AND  create_time>'{} 00:00:00' AND   create_time<'{} 23:59:59'    AND nick_name IS NOT NULL  AND open_id IN(SELECT DISTINCT open_id FROM voyager.`wx_user_authorize_log` WHERE TYPE =1 AND operate=1 ))
                 AND  create_time>'{} 00:00:00' AND   create_time<'{} 23:59:59'  '''.format(self.source_id,self.begin_time,self.begin_time,self.begin_time,self.begin_time)
